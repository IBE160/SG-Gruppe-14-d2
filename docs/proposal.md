...
3.  **Negotiate** with these suppliers because their initial quotes will cumulatively exceed the project's strict budget (700 MNOK) and timeline (15 months).
...
### Frontend
- **Framework:** Next.js 14+ (App Router) for a responsive, server-side rendered React application.
- **Language:** TypeScript.
...
- **Database:** **Supabase** (PostgreSQL). Used for Auth, Persistence, and Real-time updates (if needed for chat).
- **AI Integration:**
    *   **Service:** OpenAI API (GPT-4o) or Gemini Pro.
    *   **Logic:** The FastAPI backend will act as the orchestrator. It will hold the "System Prompts" for each supplier persona (e.g., "You are a stubborn architect who cares about aesthetics over budget...").
